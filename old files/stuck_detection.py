...
last_position = (0, 0)  # Last recorded position
time_last_moved = 0     # Time of last significant movement
...
def handle_stuck_situation(pca, rgb_led_instance, movement, dead_reckoning):
    ...
    if movement.are_motors_commanded_to_move() and (time.time() - time_last_moved > c.MOVEMENT_SETTINGS["STUCK_TIME"]):
        ...
        if distance_moved < c.MOVEMENT_SETTINGS["STUCK_DISTANCE"]:
            ...
            time_last_moved = time.time()  # Update the time of last significant movement
        
        last_position = position  # Update last position for next check
    else:
        # Reset the variables if no stuck situation is detected
        stuck_count = 0
